...
gwasfile="supp_note_data/eqtl_catalogue/ge.filter_gene.clumped.assoc.txt" #data on Zenodo
...
dx=d_gwas_annots_norm %>% select(colnames(d_gwas_annots_norm)[1:6])
...
compute_reg <- function(d_test_glm,d2){
	...
	d1=d_test_glm %>% select(-study,-eGeneSymbol,-pip,-z,-type)
	d1$type=1
	dx=rbind(d1,d2)
...
d2=d2[sample(1:nrow(d2),20000,replace=T),]
d2$type=0

###excluding GTEx
d_no_GTEx=d_eqtl_eGene[!(grepl("GTEx", d_eqtl_eGene$study, fixed = TRUE)),]
studies=sort(unique(d_no_GTEx$study))
d_out=d_out[d_out$study=="X",]

#loop over stduies
for (study_temp in studies){

	d_test_glm=d_no_GTEx[d_no_GTEx$study==study_temp,]
	print(paste0(study_temp,": ",as.character(nrow(d_test_glm))))

	d_out_temp=compute_reg(d_test_glm,d2)
	d_out_temp$study=study_temp

	d_out=rbind(d_out,d_out_temp)

}

outfile="extended_eqtl_analyses/eqtlcatalouge/ge_eqtls.noGTEx.regression_results.txt"
write.table(d_out,file=outfile,quote=F,sep=",",row.names=F)

###only GTEx

d_GTEx=d_eqtl_eGene[(grepl("GTEx", d_eqtl_eGene$study, fixed = TRUE)),]
studies=sort(unique(d_GTEx$study))
d_out=d_out[d_out$study=="X",]

#loop over stduies
for (study_temp in studies){

	d_test_glm=d_GTEx[d_GTEx$study==study_temp,]
	print(paste0(study_temp,": ",as.character(nrow(d_test_glm))))
...
outfile="extended_eqtl_analyses/eqtlcatalouge/ge_eqtls.GTEx.regression_results.txt"
write.table(d_out,file=outfile,quote=F,sep=",",row.names=F)
