...
d_data=fread("extended_gwas_analyses/cS2G/finemapping_cS2G_UKBB_PIP05.annot") #data from Gazal et al.
d_data=d_data[d_data$gene %in% d_pc_genes$gene,]

#print stats
d_data_filter=d_data[d_data$PIP>0.5,] #filter high confidence SNPs
d_data_filter$SNP_Gene=paste0(d_data_filter$SNP,"_",d_data_filter$gene)
d_data_filter$SNP_Gene_trait=paste0(d_data_filter$SNP,"_",d_data_filter$gene,"_",d_data_filter$DISEASE.TRAIT)

length(unique(d_data_filter$SNP_Gene))
length(unique(d_data_filter$SNP_Gene_trait))
length(unique(d_data_filter$DISEASE.TRAIT))

#######

traits=unique(d_data$DISEASE.TRAIT)

#loop over traits
kk=1
trait_temp=traits[kk]
print(paste0(as.character(kk),":",trait_temp))
d_data_temp=d_data[d_data$DISEASE.TRAIT==trait_temp & d_data$PIP>0.5,] #filter high confidence SNPs
d_data_temp=d_data_temp %>% arrange(-cS2G)
d_data_temp=d_data_temp[!duplicated(d_data_temp$SNP),] 
d2=d_gene_annots_norm[!(d_gene_annots_norm$gene %in% d_data_temp$gene),]; d2$type=0
d1=d_gene_annots_norm[(d_gene_annots_norm$gene %in% d_data_temp$gene),]; d1$type=1
...
d_data_cumulative$SNP_Gene=paste0(d_data_cumulative$SNP,"_",d_data_cumulative$gene)
d_data_cumulative$SNP_Gene_trait=paste0(d_data_cumulative$SNP,"_",d_data_cumulative$gene,"_",d_data_cumulative$DISEASE.TRAIT)

#print stats
length(unique(d_data_cumulative$SNP_Gene))
length(unique(d_data_cumulative$SNP_Gene_trait))
length(unique(d_data_cumulative$DISEASE.TRAIT))


#save

outfile="extended_gwas_analyses/cS2G/cS2G.regression_results.txt"
write.table(d_out,file=outfile,quote=F,sep="\t",row.names=F)

outfile="extended_gwas_analyses/cS2G/cS2G.SNP_gene_links.txt"
