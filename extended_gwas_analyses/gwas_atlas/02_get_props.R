...
d2=d2[sample(1:nrow(d2),100000,replace=T),] #random set of SNPs as control
d2$type=0

d_test_glm=d_gwas_annots_norm[d_gwas_annots_norm$id %in% (d_pheno[d_pheno$no_rep_phenos==1,]$id),]
d_out1=compute_reg(d_test_glm,d2)
d_out1$trait_cat="no_rep_phenos"

d_test_glm=d_gwas_annots_norm[d_gwas_annots_norm$id %in% (d_pheno[d_pheno$no_rep_disease==1,]$id),]
d_out2=compute_reg(d_test_glm,d2)
d_out2$trait_cat="no_rep_disease"

d_test_glm=d_gwas_annots_norm[d_gwas_annots_norm$id %in% (d_pheno[d_pheno$no_rep_disease_disorder==1,]$id),]
d_out3=compute_reg(d_test_glm,d2)
d_out3$trait_cat="no_rep_disease_disorder"

d_test_glm=d_gwas_annots_norm[d_gwas_annots_norm$id %in% (d_pheno[d_pheno$indep_phenos==1,]$id),]
d_out4=compute_reg(d_test_glm,d2)
d_out4$trait_cat="indep_phenos"

d_test_glm=d_gwas_annots_norm[d_gwas_annots_norm$id %in% (d_pheno[d_pheno$indep_disease==1,]$id),]
d_out5=compute_reg(d_test_glm,d2)
d_out5$trait_cat="indep_disease"

d_test_glm=d_gwas_annots_norm[d_gwas_annots_norm$id %in% (d_pheno[d_pheno$indep_disease_disorder==1,]$id),]
...
outfile=paste0(atlasdir,"GWAS.atlas.regression_results.txt")
