...
gwasfile="other_qtls/ieqtls/ieqtls.clumped.txt"
...
d_gwas=d_gwas[d_gwas$GeneSymbol %in% d_gene$GeneSymbol,]
d_gwas=d_gwas[d_gwas$SNP %in% d_all$SNP,]
colnames(d_gwas)[3]="eGeneSymbol"
...
d_eqtl=dx %>% arrange(Pval)

d_eqtl$`Tissue.Cell`=paste0(d_eqtl$Tissue,".",d_eqtl$Cell)
d_eqtl_top=d_eqtl[d_eqtl$Pval<(5e-6),]


tx_temp=aggregate(d_eqtl$SNP,list(d_eqtl$`Tissue.Cell`),length)
colnames(tx_temp)=c("Tissue.Cell","count")
tx_temp=tx_temp %>% arrange(-count)
...
compute_reg <- function(d_test_glm,d2){
	...
	d1=d_test_glm %>% select(-Tissue,-eGeneSymbol,-Pval,-Cell,-"Tissue.Cell")
	d1$type=1
	dx=rbind(d1,d2)
...
######################
#regression models by tissue/cell type pair
...
d_test_glm=d_eqtl_top
d_out=compute_reg(d_test_glm,d2)
d_out$Tissue.Cell="All-top SNPs"

###

tissue_cells=tx_temp$Tissue.Cell

for (tissue_cell_temp in tissue_cells){

	d_test_glm=d_eqtl[d_eqtl$Tissue.Cell==tissue_cell_temp,]
	print(paste0(tissue_cell_temp,": ",as.character(nrow(d_test_glm))))

	d_out_temp=compute_reg(d_test_glm,d2)
	d_out_temp$Tissue.Cell=tissue_cell_temp

	d_out=rbind(d_out,d_out_temp)

}

outfile="other_qtls/ieqtls/ieqtl.regression_results.txt"
