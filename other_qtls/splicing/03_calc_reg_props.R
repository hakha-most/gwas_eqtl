...
infofile="snp_annotations/filter_snps_set2.txt"
...
gwasfile="other_qtls/splicing/sqtls.clumped.txt"
d_gwas=fread(gwasfile)
colnames(d_gwas)[1]="Tissue"
colnames(d_gwas)[2]="sGeneSymbol"
...
dx$GeneSymbol=dx$sGeneSymbol
dx=left_join(dx,(d_gene_norm %>% select(-gene,-TSSD)),by="GeneSymbol")
dx=left_join(dx,d2,by="SNP")
d_sqtl_sGene=dx %>% arrange(Pval)

d_sqtl_sGene_top=d_sqtl_sGene[d_sqtl_sGene$Pval<(5e-8),]
...
compute_reg <- function(d_test_glm,d2){
	...
	d1=d_test_glm %>% select(-Tissue,-sGeneSymbol,-Pval,-rank)
	d1$type=1
	dx=rbind(d1,d2)
...
######################
#logistic regression per tissue
...
###
#loop over tissues
tissues=sort(unique(d_gwas$Tissue))

tissue_temp=tissues[1]
d_test_glm=d_sqtl_sGene_top[d_sqtl_sGene_top$Tissue==tissue_temp,]
print(paste0(tissue_temp,": ",as.character(nrow(d_test_glm))))
d_out_temp=compute_reg(d_test_glm,d2)
d_out_temp$Tissue=tissue_temp
d_out=d_out_temp

for (tissue_temp in tissues[2:length(tissues)]){
...
outfile="other_qtls/splicing/sqtls.regression_results.txt"
